...
exports.generatePlanViewHtml = void 0;
...
function generatePlanViewHtml(planPath, rootPath) {
    ...
    // Load the plan file
    const { content, metadata } = (0, markdownParser_1.loadPlanFile)(planPath);
    ...
    const headers = (0, markdownParser_1.extractMarkdownHeaders)(content);
    const tocHtml = (0, markdownParser_1.generateTableOfContents)(headers);
    // Check status of other phases
    const phaseStatus = (0, markdownParser_1.checkPlanPhaseStatus)(planPath);
    // Extract plan name for display, using the improved extraction function
    const filename = path.basename(planPath);
    const planNameRaw = (0, markdownParser_1.extractMarkdownHeaders)(content)[0]?.text || 'Unknown Plan';
    ...
    const processedContent = (0, markdownParser_1.parseMarkdownForInteractivity)(content);
    ...
    return (0, navigation_1.getBaseHtml)(`Claude Cascade - ${metadata.title || cleanFilename}`, allStyles, body);
}
exports.generatePlanViewHtml = generatePlanViewHtml;
