...
export class PhaseChart {
    ...
    // Initialize chart
    initialize(container: HTMLElement, data: PhaseData[]) {
        this.container = container;
        this.data = data;
        
        this.render();
        
        logActivity('phaseChartInitialized', { 
            id: this.id, 
    ...
    private render() {
        ...
        const wrapper = createSafeElement('div', '', 'aurora-chart-wrapper');
    ...
    // Render bar chart
    private renderBarChart(wrapper: HTMLElement) {
        // Create SVG
    ...
    // Render line chart
    private renderLineChart(wrapper: HTMLElement) {
        // Create SVG
        this.svgElement = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
        this.svgElement.setAttribute('width', this.options.width.toString());
        this.svgElement.setAttribute('height', this.options.height.toString());
        ...
        // Create line path
        let pathData = '';
        const points: Array<{x: number, y: number, data: PhaseData}> = [];
    ...
    // Render area chart
    private renderAreaChart(wrapper: HTMLElement) {
    ...
    // Render donut chart
    private renderDonutChart(wrapper: HTMLElement) {
    ...
    private createDonutSegment(cx: number, cy: number, outerRadius: number, innerRadius: number, startAngle: number, endAngle: number): string {
    ...
    private getPhaseColors(status: string): { start: string; end: string } {
    ...
    private createLegend(): HTMLElement {
        const legend = createSafeElement('div', '', 'aurora-chart-legend');
        
        this.data.forEach((phase, index) => {
            const item = createSafeElement('div', '', 'aurora-chart-legend-item');
            
            const dot = createSafeElement('span', '', 'aurora-chart-legend-dot');
            const colors = this.getPhaseColors(phase.status);
            dot.style.background = `linear-gradient(135deg, ${colors.start}, ${colors.end})`;
            
            const label = createSafeElement('span', phase.name, 'aurora-chart-legend-label');
            const value = createSafeElement('span', `${phase.completed}/${phase.total}`, 'aurora-chart-legend-value');
    ...
    // Show tooltip
    private showTooltip(phase: PhaseData, x: number, y: number) {
        // Remove existing tooltip
        this.hideTooltip();
        
        const tooltip = createSafeElement('div', '', 'aurora-chart-tooltip');
    ...
    // Update chart data
    updateData(data: PhaseData[]) {
        this.data = data;
        this.render();
        
        logActivity('phaseChartUpdated', { 
            id: this.id, 
            dataPoints: data.length 
        });
    }
    ...
    // Change chart type
    setType(type: 'bar' | 'line' | 'area' | 'donut') {
    ...
    destroy() {
        ...
        logActivity('phaseChartDestroyed', { id: this.id });
    }
}
