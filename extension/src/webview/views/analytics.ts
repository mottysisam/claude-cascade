...
function calculateAnalyticsData(rootPath: string) {
...
export function generateAnalyticsHtml(rootPath: string): string {
    ...
    return getBaseHtml('Claude Cascade - Analytics', analyticsStyles, body);
}

// Helper to render the completion trend chart
function renderCompletionTrendChart(trend: Record<string, number>): string {
...
function renderPhaseBars(phase1: number, phase2: number, phase3: number): string {
...
function getCompletionClass(rate: number): string {
    if (rate >= 80) return 'value-good';
    if (rate >= 50) return 'value-warning';
    return 'value-error';
}