...
import { getMarkdownPreview, checkPlanPhaseStatus } from '../utils/markdownParser';

// Get plans for a specific phase
function getPhasePlans(rootPath: string, phase: string) {
    ...
    const plans = files.map(file => {
        ...
        const preview = getMarkdownPreview(content, 150);
        
        // Get status of related phases
        let phaseStatus = {
            phase1: true, // We assume the current file exists
            phase2: false,
            phase3: false
        };
        
        if (phase === 'pre-execution') {
            const status = checkPlanPhaseStatus(filePath);
...
function extractPlanName(filename: string): string {
...
export function generatePhaseTableHtml(phase: string, rootPath: string): string {
    ...
    return getBaseHtml(`Claude Cascade - ${phaseTitle}`, tablesStyles, body);
}

// Helper to render status indicator
function renderStatusIndicator(status: boolean, label: string, isCurrentPhase: boolean) {
    if (isCurrentPhase) {
