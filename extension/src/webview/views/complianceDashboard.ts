 ...
 */
export function generateComplianceDashboardHtml(rootPath: string): string {
    ...
    return getBaseHtml('Claude Cascade - Compliance Dashboard', styles, body);
}
 ...
 */
function generateDashboardData(rootPath: string): any {
 ...
 */
function calculateTimeDifference(start: Date, end: Date): number {
 ...
 */
function getComplianceDescription(rate: number): string {
    if (rate >= 90) {
 ...
 */
function generatePlanRows(plans: any[]): string {
    if (plans.length === 0) {
 ...
 * @returns HTML string
 */
function renderPhaseStatus(exists: boolean, phase: number): string {
    if (exists) {
 ...
 * @returns HTML string
 */
function generateViolationsSection(violations: any[]): string {
